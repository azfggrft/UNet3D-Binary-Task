...
class ModelInference:
    ...
    def infer_folder(self, images_folder, masks_folder=None, target_size=(64, 64, 64), 
        ...
        if all_dice_scores:
            # 收集每個類別的 Dice
            num_classes = self.config.get('n_classes', 4)
            per_class_dices = {cls: [] for cls in range(1, num_classes)}
            
            for result in all_results:
                if 'metrics' in result:
                    for i, d in enumerate(result['metrics'].get('dice_per_class', [])):
                        cls = i + 1
                        if cls in per_class_dices:
                            per_class_dices[cls].append(d)
            ...
            summary = {
                ...
                'per_class_avg_dice': {                          # ← 新增
                    f'class_{cls}': float(np.mean(dices)) if dices else 0.0
                    for cls, dices in per_class_dices.items()
                },
            ...
            for cls, avg in summary['per_class_avg_dice'].items():  # ← 新增
                print(f"  {cls} 平均 Dice: {avg:.6f}")
